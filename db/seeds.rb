...
# 			puts "-----------------------------\nPage#{page}"
# 			puts "Extracting!"
# 			data = RestClient.get("https://maps.googleapis.com/maps/api/place/nearbysearch/json?key=AIzaSyAEWhLSygrd0gCgpyC0DloRAcvWgB9Ws_w&location=-33.922015,18.419624&radius=50000&sensor=false#{minprice}#{maxprice}&types=#{tag}#{pagetoken}")
...
while true
	...
	while true
		...
		data = RestClient.get("https://maps.googleapis.com/maps/api/place/nearbysearch/json?key=#{api_key}&location=#{lat},#{long}&rankby=distance&types=#{tags}&sensor=false#{pagetoken}&minprice=0&maxprice=0")
